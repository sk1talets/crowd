...
class UserTest < ActiveSupport::TestCase
    PASSWORD = "teSt#1"  
    def setup
        @user = User.new(name: "user", password: PASSWORD, password_confirmation: PASSWORD, role: "user")
        @admin = User.new(name: "admin", password: PASSWORD, password_confirmation: PASSWORD, role: "admin")
    end
    ...
    test "check user role" do
        assert_not @user.admin?
        assert @admin.admin?
    end
    
    test "check user default role" do
        user_= User.new(name: "user", password: PASSWORD, password_confirmation: PASSWORD)
        assert user_.user?
    end
